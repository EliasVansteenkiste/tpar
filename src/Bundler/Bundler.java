...
public class Bundler {
	...
	public Bundler(Circuit c){
		this.c = c;
		sourceBundlesCPY = new HashMap<Pin,Set<Connection>>(c.sourceBundles);
		sinkBundlesCPY = new HashMap<Pin,Set<Connection>>(c.sinkBundles);
...
//		printBundles();
	...
	private void findNextCluster(Set<Connection> consInCluster,
		...
		qe.add(Collections.min(consCPY));
		...
		while (!qe.isEmpty()) {
			...
			Set<Connection> sinkBundle = sinkBundlesCPY.remove(con.sink);
	...
	private class TmpBundleComparator implements Comparator<Set<Connection>>{
		@Override
		public int compare(Set<Connection> o1, Set<Connection> o2) {
			if(o1.size() < o2.size())
				return -1;
				...
				ArrayList<Connection> m1 = new ArrayList<Connection>(o1);
				ArrayList<Connection> m2 = new ArrayList<Connection>(o2);
				Collections.sort(m1);
				Collections.sort(m2);
				for(int i = 0; i<m1.size(); i++) {
					int r = m1.get(i).compareTo(m2.get(i));
					if(r!=0)
						return r;
				}
...
//		Collections.sort(maxBundles, new TmpBundleComparator());
		return maxCons;
	}
...
//			System.out.println("colorsAvailable: "+colorsAvailable);
	...
	private void removeStaticNetsFromProblem() {
		...
		//Something else
		for(Pin sink:c.sinkBundles.keySet()){
...
//		int nrOfCons = 0;
		ArrayList<Set<Connection>> bundleSetsList = new ArrayList<Set<Connection>>(bundleSets);
		Collections.sort(bundleSetsList, new TmpBundleComparator());
		for(Set<Connection> cset:bundleSetsList){
			bundleNr++;
			System.out.println("Bundle "+bundleNr+":");
			ArrayList<Connection> csetList = new ArrayList<Connection>(cset);
			Collections.sort(csetList);
			for(Connection con:csetList){
